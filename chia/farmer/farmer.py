...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        ...
        if "xvm_target_address" not in self.config:
            self.config = config["farmer"]
        if "xvm_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "xvm_target_address" not in self.config or "xvm_target_address" not in self.pool_config:
